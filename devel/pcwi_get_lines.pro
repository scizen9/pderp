; $Id: pcwi_get_lines.pro,v 1.1 2014/10/17 17:44:14 neill Exp $
;
...
;
; CALLING SEQUENCE:
;	PCWI_GET_LINES, Wave, ArcSpec, Xcen
;
; INPUTS:
...
;-
pro pcwi_get_lines,wave,spec,xcen,pkint,pgeom,watl,barno, $
	...
	pixpos = findgen(npix)
	w0 = poly(pixpos+pgeom.baroff[barno],pgeom.rbcoeffs)
	...
	for i=0,np-1 do begin
		...
		if min(pindx) gt wid+1 and $
		   max(pindx) lt npix-(wid+1) then begin
			...
			wxx   = poly(xx+pgeom.baroff[barno],pgeom.rbcoeffs)
			offs  = abs(watl - wxx)
