; $Id: pcwi_test_std.pro | Fri Jan 22 10:20:48 2016 -0800 | Don Neill  $
;
...
;-
pro pcwi_test_std,imno,ps=ps,verbose=verbose,display=display
	;
	; setup
	pre = 'PCWI_TEST_STD'
	version = repstr('$Revision: v0.2.10-21-g9ba58ad $ $Date: Fri Jan 22 10:20:48 2016 -0800 $','$','')
	q=''
	;
	; check input
	...
	;
	; log
	pcwi_print_info,ppar,pre,version
	;
	...
	icub = pcwi_read_image(pcfg.imgnum,ppar,'_icubes',hdr,/calib,status=stat)
	...
	pcwi_print_info,ppar,pre,'testing inverse sensitivity curve for '+sname
	;
	; get size
	sz = size(icub,/dim)
	;
	...
	;
	; log results
	pcwi_print_info,ppar,pre,'Invsens. Pars: X0, X1, Y0, Y1, Wav0, Wav1', $
	...
	sfw = sfw[sroi]
	fwhm = max(sfw)
	pcwi_print_info,ppar,pre,'reference spectrum FWHM used',fwhm, $
		format='(a,f5.1)'
	;
	...
	pcwi_legend,['Cal. Flux','Obs. Flux','Smoothed'],linesty=[0,0,0], $
	...
	pcwi_legend,['<Resid> = '+strtrim(string(mo[0],format='(g13.3)'),2) + $
	...
	eafil = ppar.reddir + ppar.froot + $
		string(pcfg.imgnum,format='(i0'+strn(ppar.fdigits)+')') + $
		'_ea.fits'
	if file_test(eafil) then begin
		...
		pcwi_print_info,ppar,pre,'EA file not found',eafil,/warning
	;
