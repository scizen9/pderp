; $Id: pcwi_arc_lines.pro,v 1.1 2014/10/17 17:44:11 neill Exp $
;
...
;
; CALLING SEQUENCE:
;	PCWI_ARC_LINES,Spec, Pgeom, Barno, Pixpos, WAtlas, WCntrd
;
; INPUTS:
;	Ppar	- PCWI_PPAR pipeline parameters struct
...
;-
;
pro pcwi_arc_lines,ppar,spec,pgeom,barno,pixpos,watlas,wcntrd, $
...
if n_params(0) lt 5 or keyword_set(help) then begin
	print,pre+': Info - Usage: '+pre+', Ppar, ArcSpec, Pgeom, Barno, Pixpos, WAtlas, WCntrd'
	return
endif
;
; check spec
...
if barno lt 0 or barno gt 119 then begin
	pcwi_print_info,ppar,pre,'bar number out of range (0-119)',barno,/error
	return
endif
;
; keywords
...
;
; get bar offset
baroff = pgeom.baroff[barno]
;
; read reference spectrum
rdfits1dspec,!PCWI_DATA+'thar.fits',refw,reflxo, $
...
if ngood le 0 then begin
	pcwi_print_info,ppar,pre,'The atlas specturm is lacking in data in wavelength range',/error
	return
endif
ref_spec = reflx[good]
ref_wave = refw[good]
...
if ref_npks eq 0 then begin
	pcwi_print_info,ppar,pre,'not enough good atlas peaks',/error
	return
endif
;
