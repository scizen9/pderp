; $Id: pcwi_solve_thar.pro,v 1.1 2014/10/17 17:44:13 neill Exp $
;
...
;
; CALLING SEQUENCE:
;	PCWI_SOLVE_THAR, Specs, Pgeom, Ppar
;
; INPUTS:
;	Specs	- a array of arc spectra produced by PCWI_EXTRACT_ARCS
...
;
; SIDE EFFECTS:
;	Modifies PCWI_GEOM struct by updating the bar coeffiecients and
...
;	2014-MAR-19	Use PCWI_CLEAN_COEFFS to fix errant bars
...
;	2014-SEP-11	Converted function to pro with status in pgeom.status
;	2014-SEP-16	Single iteration mode for assessing rms of fits (N&S)
;	2016-MAR-03	Split from kderp to pderp
;-
;
pro pcwi_solve_thar, specs, pgeom, ppar, tweak=tweak, plot_file=plot_file

pre = 'PCWI_SOLVE_THAR'
version = repstr('$Revision: 1.1 $ $Date: 2014/10/17 17:44:13 $','$','')
q=''
;
...
gratanom = pgeom.gratanom
;
; log info
pcwi_print_info,ppar,pre,version,/info
pcwi_print_info,ppar,pre,'img, grat, filt, nasmsk, cwave', $
...
rho = pgeom.rho
slant = pgeom.slant
lastdegree = pgeom.lastdegree
;
; which is the reference bar?
refbar = pgeom.refbar
;
; load the reference atlas spectrum.
rdfits1dspec,pgeom.refspec,refwvl,thar, $
...
if strtrim(strupcase(grating),2) eq 'MEDREZ' then begin
	; preliminary beta
	prelim_alpha = -((-264500.0)-pgeom.gratpos)/2000.0
...
pcwi_print_info,ppar,pre,'Initial calculated dispersion (A/binned pixel)', $
...
if nqwvl eq 0 then begin
	pcwi_print_info,ppar,pre,'Did not find ThAr data to match to',/error
	pgeom.status=3
	return
endif
;
...
if skymod gt 0. and skysig gt 0. and skymod-2.*skysig gt 0. then begin
	...
	pcwi_print_info,ppar,pre,'subtracting scattered light offset of', $
...
pcwi_xspec,prelim_intspec,prelim_refspec,ppar,prelim_offset,prelim_value, $
...
p0 = cwvl + pgeom.baroff*prelim_disp - prelim_offset * refdisp
;
...
pcwi_print_info,ppar,pre,'N disp. samples: ',nn
;
; which are:
...
pcwi_print_info,ppar,pre,'Using fits of degree',degree,/info
;
...
for b = 0,119 do begin
	...
	for d = 0, nn do begin
		...
		if nqwvl eq 0 then begin
			pcwi_print_info,ppar,pre,'Insufficient reference wavelengths',/err
			pgeom.status=4
			return
		endif
		;
		...
		pcwi_xspec,intspec,subrefspec,ppar,soffset,svalue, $
	...
	;
	pcwi_print_info,ppar,pre,'Initial Fit: Bar#,Cdisp,Coefs',b,bardisp[b],scoeff[0:3], $
...
if ppar.cleancoeffs then $
	pcwi_clean_coeffs,intcoeff,degree,ppar
;
...
; most 1/3 of the CCD 
if keyword_set(tweak) then begin
	pcwi_print_info,ppar,pre,'using '+strn(n_elements(edgespace))+' iterations to tweak higher order terms'
	;
	...
	pcwi_print_info,ppar,pre,'Spectral line finding/matching params: PkSig, PkDel, PkIso', $
	...
	if nqwvs eq 0 then begin
		pcwi_print_info,ppar,pre,'No wavelength overlap with atlas spectrum',/error
		pgeom.status=5
		return
	endif
	...
	if twk_ref_npks eq 0 then begin
		pcwi_print_info,ppar,pre,'No good atlas points found',/error
		pgeom.status=6
		return
	endif
	pcwi_print_info,ppar,pre,'Number of clean atlas lines found',twk_ref_npks,$
		format='(a,i4)'
	;
	if ppar.display ge 2 then begin
	...
	if nasmask and niter gt 1 then $
		pcwi_print_info,ppar,pre,'tweaking not recommended when the NS mask is in',/warning
	;
	...
	for iter = 0,niter-1 do begin
		...
		for b=0, 119 do begin
			...
			if ngoodpeaks eq 0 then begin
				pcwi_print_info,ppar,pre,'not enough good spec peaks!',/error
				pgeom.status=7
				wset,0
				return
			endif
			...
			delta = abs(fix(2.5*pgeom.resolution/twkcoeff[1,b]))
			...
			if ngoodsigma eq 0 then begin
				pcwi_print_info,ppar,pre,'No good spec points found',/error
				pgeom.status=8
				wset,0
				return
			endif
			...
			if nmatchedpeaks le 2 then begin
				pcwi_print_info,ppar,pre,'Not enough peaks matched in Bar # '+strn(b)+': '+strn(nmatchedpeaks),/error
				pgeom.status=9
				wset,0
				return
			endif
			...
			if iter eq niter-1 then begin
				...
				pcwi_print_info,ppar,pre,'Tweaked Fit: Bar#,Npks,Sig,Cdisp,Coefs', $
		...
		pcwi_print_info,ppar,pre,"Iteration "+string(iter+1,"(i2)")+" of "+ string(niter,"(i2)")+" done."
		;
		; now clean each slice of outlying bars
		if ppar.cleancoeffs then begin
			...
			wset,0
			pcwi_clean_coeffs,twkcoeff,degree,ppar,plot=clnplot
		endif
	endfor; iter
endif; tweak
; endtweak
...
if keyword_set(plot_file) then begin
	psfile,plot_file
	pcwi_print_info,ppar,pre,'plotting diagnostics to: '+plot_file+'.ps'
endif
deepcolor
!p.background=colordex('white')
!p.color=colordex('black')
...
if keyword_set(tweak) and niter gt 1 then begin 
	...
	mom = moment(sigmas)
	pgeom.avewavesig = mom[0]
	pgeom.stdevwavesig = sqrt(mom[1])
	...
	pcwi_oplot_slices
	pcwi_print_info,ppar,pre,'average bar wavelength sigma (Ang)',mom[0],' +- ',sqrt(mom[1]),$
		format='(a,f7.3,a,f7.3)'
	pcwi_print_info,ppar,pre,'min/max bar wavelength sigma (Ang)',minmax(sigmas), $
		format='(a,f7.3,2x,f7.3)'
	if noutliers gt 0 then $
		pcwi_print_info,ppar,pre,'> 3sig outlier bars present, may want to tweak ppar.pkiso: Imgnum, Bars', $
...
t1wvs = fltarr(120)
for b=0,119 do begin
	wy0 = poly(pgeom.goody0,fincoeff[*,b])
	y0wvs[b] = wy0
	wy1 = poly(pgeom.goody1,fincoeff[*,b])
	y1wvs[b] = wy1
	wt0 = poly(pgeom.trimy0,fincoeff[*,b])
	t0wvs[b] = wt0
	wt1 = poly(pgeom.trimy1,fincoeff[*,b])
	t1wvs[b] = wt1
endfor
...
;
; now let's update pgeom struct
;
; order of the fits
pgeom.bfitord = degree+1
;
; reference bar coeff's
pgeom.rbcoeffs = fincoeff[*,refbar]
;
; inclusive, all, and trim wavelengths
pgeom.waveall0 = wavall0
pgeom.waveall1 = wavall1
pgeom.wavegood0 = wavgood0
pgeom.wavegood1 = wavgood1
;
; make sure wavelengths are on fiducial scale and
; zeropoint is integer number of bins from reference wl (3000A)
ndels = long((trim0 - pgeom.wavefid)/pgeom.dwout)
pgeom.wave0out = pgeom.wavefid + float(ndels) * pgeom.dwout
ndels = long((trim1 - pgeom.wavefid)/pgeom.dwout)
pgeom.wave1out = pgeom.wavefid + float(ndels) * pgeom.dwout
;
; log values
pcwi_print_info,ppar,pre,'min,max good wavelengths',wavgood0,wavgood1, $
	format='(a,2f9.2)'
pcwi_print_info,ppar,pre,'min,max inclusive wavelengths',wavall0,wavall1, $
	format='(a,2f9.2)'
pcwi_print_info,ppar,pre,'min,max trim wavelengths',pgeom.wave0out,pgeom.wave1out, $
	format='(a,2f9.2)'
;
...
for b=0,119 do begin
	;
	; update coeffs in pgeom struct
	dcoeff = fincoeff[*,b]
	pcwi_apply_coeffs,pgeom,b,dcoeff
	;
	subxvals = xvals[minrow:maxrow]
