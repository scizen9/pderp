; $Id: pcwi_make_rr.pro,v 1.2 2015/02/21 00:18:39 neill Exp $
;
...
;	image from a list of rr image numbers given in PCWI_PPAR struct
;
; CATEGORY:
...
;
; INPUTS:
;	Ppar	- PCWI_PPAR struct for rr group to combine
;
; KEYWORDS:
;	None
;
...
; EXAMPLE:
;
;	ppar = PCWI_PPAR_READ('mrr_0.ppar')
;	PCWI_MAKE_RR,ppar
;
; TODO:
...
;-
pro pcwi_make_rr,ppar
	;
	; version
	version = repstr('$Revision: 1.2 $ $Date: 2015/02/21 00:18:39 $','$','')
	;
	; initialize
	pre = 'PCWI_MAKE_RR'
	;
	...
	if strlen(ppar.rrs) le 0 then begin
		pcwi_print_info,ppar,pre,'no relative responses entered',/error
		return
	endif
	;
	; directories
	...
	pcwi_print_info,ppar,pre,'combining this many images',nr
	;
	; read first image
	...
	if file_test(rfil) then begin
		...
		pcwi_print_info,ppar,pre,'Image file not found',rfil, $
			format='(a,a)',/error
		return
	endelse
	;
	...
	if nf eq 2 then begin
		...
		if file_test(rfil) then begin
			...
			pcwi_print_info,ppar,pre,'Image file not found', $
				rfil,/error
			return
		endelse
		;
		...
		if sz[0] ne nx or sz[1] ne ny then begin
			pcwi_print_info,ppar,pre,'Size mismatch',rfil,/error
			return
		endif
		;
		; simple average
		...
		for i=1,nf-1 do begin
			...
			if file_test(rfil) then begin
				...
				pcwi_print_info,ppar,pre,'File not found', $
					rfil,/error
				return
			endelse
			;
			...
			sz = size(im,/dim)
			if sz[0] ne nx or sz[1] ne ny then begin
				pcwi_print_info,ppar,pre,'Size mismatch', $
					rfil,/error
				return
			endif
			;
	...
	pcwi_write_image,mrr,hdr,ppar.masterrr,ppar
	;
	return
end
