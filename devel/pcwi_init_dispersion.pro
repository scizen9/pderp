; $Id: pcwi_init_dispersion.pro,v 1.1 2014/10/17 17:44:11 neill Exp $
;
; Copyright (c) 2016, California Institute of Technology. All rights
;	reserved.
;+
; NAME:
;	PCWI_INIT_DISPERSION
;
; PURPOSE:
...
;
; CALLING SEQUENCE:
;	PCWI_INIT_DISPERSION,Spec, Pgeom
;
; INPUTS:
...
;
; SIDE EFFECTS:
;	Updates Pgeom.rbcoeffs to contain the reference bar wavelength coeffs
;
; PROCEDURE:
;
; EXAMPLE:
...
;	pcwi_init_dispersion,arcspec,pgeom,ppar
;
...
;	2013-DEC-15	Uses pcwi_sort_atlas to indicate good atlas lines
;	2016-MAR-03	Split from kderp to pderp
;-
;
pro pcwi_init_dispersion, spec, pgeom, ppar, help=help
;
; startup
pre = 'PCWI_INIT_DISPERSION'
version = repstr('$Revision: 1.1 $ $Date: 2014/10/17 17:44:11 $','$','')
q = ''
;
; check inputs
if n_params(0) lt 3 or keyword_set(help) then begin
	print,pre+': Info - Usage: '+pre+', ArcSpec, Pgeom, Ppar'
	return
endif
;
; check spec
...
if ssz[0] ne 2 or ssz[2] ne 120 then begin
	print,pre+': Error - Input spec array malformed, run PCWI_EXTRACT_ARCS first.'
	return
endif
;
; Check structs
...
if pgeom.refbar lt 0 or pgeom.refbar gt 119 then begin
	pcwi_print_info,ppar,pre,'Reference bar number out of range (0-119)', $
		pgeom.refbar,/error
	return
endif
barno = pgeom.refbar
...
if pgeom.halfwidth ge 0 and pgeom.halfwidth lt 10 then $
	hw = pgeom.halfwidth $
else	hw = 3
;
; get resolution sigma
if pgeom.resolution ge 0. and pgeom.resolution lt 10. then $
	res = pgeom.resolution $
else	res = 0.5
;
; get fit order
...
while not file_test(pgeom.refspec) do begin
	print,pre+': Error reference spectrum not found: ',pgeom.refspec
	read,'Enter full file spec for reference spectrum: ',q
	pgeom.refspec = strtrim(q,2)
endwhile
rdfits1dspec,pgeom.refspec,refw,reflxo,deltawave=refdw
...
;
; get line list
while not file_test(pgeom.reflist) do begin
	print,pre+': Error reference line list not found: ',pgeom.reflist
	read,'Enter full file spec for reference line list: ',q
	pgeom.reflist = strtrim(q,2)
endwhile
;readcol,pgeom.reflist,atlas_w,atlas_id,format='d,a',/silent
;
; set up plot
deepcolor
...
;
; wavelength range
cfg = pcwi_read_cfg(pgeom.arcfname)
pcwi_print_cfgs,cfg,/header
wave0 = wc0 - pgeom.wavran/2.
wave1 = wc0 + pgeom.wavran/2.
xrng=[0,npspec-1]
rrng=[wave0,wave1]
xspec = lindgen(npspec)
;
; check for nod-and-shuffle
if pgeom.nasmask eq 1 then begin
	;
	; update ranges
...
if ngood le 0 then begin
	pcwi_print_info,ppar,pre,'The atlas spectrum is lacking in data in wavelength range',/error
	return
endif
ref_spec = reflx[good]
ref_wave = refw[good]
...
if bar_npks le 0 then begin
	pcwi_print_info,ppar,pre,'No ref bar peaks found',/error
	return
endif
;
; now match them
...
if nmatchedpeaks le 0 then begin
	pcwi_print_info,ppar,pre,'No peaks matched!',/error
	return
endif
;
...
for i=0,n_elements(reslab)-1 do $
	pcwi_print_info,ppar,pre,reslab[i]
;
; update Pgeom
pgeom.rbcoeffs[0:fito] = coefs
;
; Pgeom timestamp
