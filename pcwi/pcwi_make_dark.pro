; $Id: pcwi_make_dark.pro | Wed Mar 4 12:02:01 2015 -0800 | Don Neill  $
;
...
;	image numbers given in PCWI_PPAR struct PPAR.DARKS.
;
; CATEGORY:
...
;
; INPUTS:
;	Ppar	- PCWI_PPAR struct for dark group to combine
;
; KEYWORDS:
;	None
;
...
; EXAMPLE:
;
;	ppar = PCWI_PPAR_READ('mdark_0.ppar')
;	PCWI_MAKE_DARK,ppar
;
...
pro pcwi_make_dark,ppar
	...
	if strlen(ppar.darks) le 0 then begin
		pcwi_print_info,ppar,pre,'no darks entered',/error
		return
	endif
	;
	; directories
	...
	if nd ge ppar.mingroupdark then begin
		...
		pcwi_print_info,ppar,pre,'combining this many images',nd
	    	;
	    	; read first image
		...
		if file_test(vfil) then begin
			...
			pcwi_print_info,ppar,pre,'Variance file not found', $
				vfil,/warning
			var = fltarr(sz)
		endelse
	    	;
		...
		if file_test(mfil) then begin
			...
			pcwi_print_info,ppar,pre,'Mask file not found', $
				mfil,/warning
			msk = intarr(sz)
		endelse
		;
		...
		;
		; write out master variance
		pcwi_write_image,mmsk,mskhdr,mastermsk,ppar
	endif else $
		pcwi_print_info,ppar,pre,'too few dark frames',nd,/error
	;
	return
end
