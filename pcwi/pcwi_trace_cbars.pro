; $Id: pcwi_trace_cbars.pro | Fri Apr 24 12:15:57 2015 -0700 | Don Neill  $
;
...
;
; CALLING SEQUENCE:
;	PCWI_TRACE_CBARS,Img,Pgeom,WarpIm
;
; INPUTS:
...
;
; OUTPUTS:
;	Pgeom	- PCWI geometric transformation struct 
;			(see PCWI_GEOM__DEFINE.PRO)
;
; SIDE EFFECTS:
;	None.
;
...
;	pcwi_trace_cbars,img,pgeom,ppar,/moffat
;
; TODO:
...
pro pcwi_trace_cbars, img, pgeom, ppar, $
...
;
; init
pre = 'PCWI_TRACE_CBARS'
q=''
status = -1
;
; check inputs
if n_params(0) lt 2 or keyword_set(help) then begin
	print,pre+': Info - Usage: '+pre+', CBarsImg, Pgeom, [Ppar]'
	return
endif
...
if do_plots ge 1 then begin
	window,0,title='pcwi_trace_cbars'
	deepcolor
...
;
; check Pgeom
if pcwi_verify_geom(pgeom,/silent) ne 0 then begin
...
;
; log
pcwi_print_info,ppar,pre,'Tracing bars in image number',pgeom.cbarsimgnum
;
...
if skysig le 0. then $
	skysig = max(row)*0.01
;
pcwi_print_info,ppar,pre,'Sky, SkySig',sky,skysig
;
...
while npks ne 120 and tries lt 10 do begin
	...
	tries += 1
endwhile
pcwi_print_info,ppar,pre,'final bar thresh, ntries',barth,tries
;
; did we succeed?
if npks ne 120 then begin
	pcwi_print_info,ppar,pre,'unable to find 120 peaks',npks,/error
	pgeom.status=1
	return
endif
pcwi_print_info,ppar,pre,'Found 120 peaks'
;
...
for j=0,npks-1 do begin
	...
	if keyword_set(gauss) or keyword_set(moffat) then begin
		...
		;
		; print results
		pcwi_print_info,ppar,pre,'Pk#, chi^2/dof, GauPk, sig, Cntr',$
	...
	; do centroid instead of fitting
		...
		;
		; print results
		pcwi_print_info,ppar,pre,'Pk#, Cntr',j,cnt, $
	...
	if do_plots ge 2 then begin
		...
		plot,x,y,psym=-4,xran=xrng,/xsty,xtitle='PIX', $
			...
			title = 'Image: '+strn(pgeom.cbarsimgnum) + $
...
; assumes pgeom.refbar is middle bar of slice (out of 5 bars per slice)
...
delx = delx / 4.0
pgeom.refdelx = delx
pgeom.refoutx = barx[refb] + (findgen(5)-2.) * delx
;
; log results
pcwi_print_info,ppar,pre,'reference delta x',pgeom.refdelx
pcwi_print_info,ppar,pre,'reference x positions',pgeom.refoutx, $
	format='(a,5f9.2)'
;
...
; loop over up,down
for k=0,1 do begin
    pcwi_print_info,ppar,pre,msg[k]
    ;
    ; loop over peaks
    for j=0,npks-1 do begin
	;
	...
	for i=p0+inc[k],lim[k],inc[k] do begin
		...
		; here we are just centroiding so no fitting
			...
			;
			; print results
			;pcwi_print_info,ppar,pre,'Bar#,Sam#,Cnt,Max[y],Pk', $
