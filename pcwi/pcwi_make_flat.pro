; $Id: pcwi_make_flat.pro | Wed Mar 4 12:02:01 2015 -0800 | Don Neill  $
;
...
;	image numbers given in PCWI_PPAR struct, then creates a flat image
...
;
; INPUTS:
;	Ppar	- PCWI_PPAR struct for flat group to combine
;
; KEYWORDS:
;	None
;
...
; EXAMPLE:
;
;	ppar = PCWI_PPAR_READ('mflat_0.ppar')
;	PCWI_MAKE_FLAT,ppar
;
; TODO:
...
pro pcwi_make_flat,ppar
	...
	if strlen(ppar.cflats) le 0 then begin
		pcwi_print_info,ppar,pre,'no flats entered',/error
		return
	endif
	;
	; directories
	...
	pcwi_print_info,ppar,pre,'combining this many images',nf
	;
	; read first image
	...
	if file_test(ffil) then begin
		...
		pcwi_print_info,ppar,pre,'Image file not found',ffil, $
			format='(a,a)',/error
		return
	endelse
	;
	...
	if nf eq 2 then begin
		...
		if sz[0] ne nx or sz[1] ne ny then begin
			pcwi_print_info,ppar,pre,'Size mismatch',ffil,/error
			return
		endif
		;
		; simple average
	...
	pcwi_write_image,flato,hdr,ppar.masterflat,ppar
	;
	return
end
