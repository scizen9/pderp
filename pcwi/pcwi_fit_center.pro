; $Id: pcwi_fit_center.pro | Fri Apr 24 15:25:00 2015 -0700 | Don Neill  $
;
...
;
; CALLING SEQUENCE:
;	PCWI_FIT_CENTER, Specs, Pgeom, Ppar, Centcoeff
;
; INPUTS:
...
;-
;
pro pcwi_fit_center, specs, pgeom, ppar, centcoeff
...
if ppar.display ge 2 then begin
	window,0,title='pcwi_fit_center'
	deepcolor
	!p.multi=0
...
degree = 4
pcwi_print_info,ppar,pre,'Using polynomial approximation of degree',degree, $
	format='(a,i5)'
;
...
if nqwvl eq 0 then begin
	pcwi_print_info,ppar,pre,'Did not find atlas data to match to',/error
	pgeom.status=2
	return
endif
;
...
if ppar.display ge 2 then begin
	...
	pcwi_legend,['Ref Bar ('+strn(refbar)+')','Atlas','CWAVE'],linesty=[0,0,2], $
...
if ddisplay then window,1,title='pcwi_xspec'
...
pcwi_print_info,ppar,pre,'Initial arc-atlas offset (px, Ang)',prelim_offset, $
...
if ppar.display ge 2 then begin
	...
	while strlen(q) gt 0 do begin
		...
		strtrim(string(prelim_offset,form='(f9.2)'),2)+' px)',/xs
...
;
; record final offset
pcwi_print_info,ppar,pre,'Final   arc-atlas offset (px, Ang)',prelim_offset, $
	prelim_offset*refdisp,format='(a,1x,f9.2,1x,f9.2)'
;
...
for b = 0,119 do begin
	...
	for d = 0, nn do begin
		...
		pcwi_xspec,intspec,subrefspec,ppar,soffset,svalue,status=status, $
	...
	;
	pcwi_print_info,ppar,pre,'Central Fit: Bar#,Cdisp,Coefs',b,bardisp[b],scoeff[0:3], $
	...
	if display then begin
		...
		pcwi_legend,['Parab: '+string(bardispp,form='(f7.4)'), $
...
if ppar.cleancoeffs then $
	pcwi_clean_coeffs,centcoeff,degree,ppar
;
; let's check the status of the fits
if total(barstat) gt n_elements(barstat)/2 then begin
	pcwi_print_info,ppar,pre,'Too many bar fit failures', $
		fix(total(barstat)),/error
	pgeom.status = 4
endif else	pgeom.status = 0
;
return
end		; pcwi_fit_center
