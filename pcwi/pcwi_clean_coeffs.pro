; $Id: pcwi_clean_coeffs.pro | Fri Apr 24 15:25:00 2015 -0700 | Don Neill  $
;
...
;
; CATEGORY:
;	Data reduction for the Palomar Cosmic Web Images (PCWI).
;
; CALLING SEQUENCE:
;	PCWI_CLEAN_COEFFS, Coeffs, Forder, Ppar
;
; INPUTS:
;	Coeffs	- 9 x 120 array of spectral fit coefficients for each bar
;	Forder	- order of spectral fit (3 - 8)
;	Ppar	- PCWI_PPAR pipeline paramter struct
;
; OUTPUTS:
;	None
;
...
;
;-
pro pcwi_clean_coeffs,coeffs,forder,ppar,plot=plot
;
; startup
pre = 'PCWI_CLEAN_COEFFS'
q = ''
;
...
if forder le 0 then begin
	pcwi_print_info,ppar,pre,'Fit order must be greater than 0', $
		forder,/error,format='(a,2x,i6)'
	return
endif
;
if forder gt 8 or forder lt 1 then $
	pcwi_print_info,ppar,pre,'Fit order warning, should be in range 1 - 8',$
				...
				if display then begin
					...
					if i eq 0 then begin
						...
						oplot,!x.crange,[statrm[0,i,j]-statrm[1,i,j],statrm[0,i,j]-statrm[1,i,j]], $
							linesty=1
					endelse
					pcwi_legend,['Original','Fixed'],psym=[4,5],symsi=[2.,2.],charsi=1.5, $
						box=0,spac=2.5
					;
					; query user
...
if modified then begin
	...
	pcwi_print_info,ppar,pre,'No cleaning required.'
endelse
;
return
end		; pcwi_clean coeffs
