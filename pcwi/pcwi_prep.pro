; $Id: pcwi_prep.pro | Thu Apr 23 11:31:25 2015 -0700 | Don Neill  $
;
...
;
; CALLING SEQUENCE:
;	PCWI_PREP, RawDir, ReducedDir, CalibDir, DataDir
;
; OPTIONAL INPUTS:
;	RawDir		- input raw directory (string) defaults to current dir
;	ReducedDir	- reduced data directory (string) defaults to './redux/'
;	CalibDir	- calib source directory (string) defaults to !PCWI_DATA + 'calib/'
;	DataDir		- PCWI data directory (string) defaults to !PCWI_DATA
;
; KEYWORDS:
; Params
...
;		1) a master pipeline parameter file, pcwi.ppar, with paramters
;			for running the pipeline.
;		2) a master link file, pcwi.link, with associations for running 
;			subsequent stages of the pipeline (PCWI_STAGE{N}).
...
;		4) a general image log file, pcwi.imlog, that records the 
;			configuration of each input raw image.  
;		5) a log file, pcwi_prep.log, that logs all steps executed in 
;			this stage and records execution time.
; NOTE:
;	The pcwi.link file can be edited to override the automated associations
...
;
; EXAMPLE:
;	Prepare to perform PCWI reductions on the images in 'night1' 
...
;	2013-SEP-13	Use PCWI_PPAR struct for subroutine parameters
...
;	2014-APR-01	Added pcwi_group_geom and processing of all calib imgs
...
;-
pro pcwi_prep,rawdir,reduceddir,calibdir,datadir, $
	froot=froot, $
	fdigits=fdigits, $
	...
	;
	; expand paths
	odir   = pcwi_expand_dir(odir)
	indir  = pcwi_expand_dir(indir)
	caldir = pcwi_expand_dir(caldir)
	ddir   = pcwi_expand_dir(ddir)
	;
	...
	; derive from file names in INDIR
		...
		if nf le 0 then begin
			pcwi_print_info,ppar,pre,'no fits files found in '+indir,/error
			return
		endif
		fdig = 0
		for i=0,nf-1 do begin
			ndig = pcwi_get_digits(flist[i])
			if ndig gt fdig then fdig = ndig
		endfor
		ppar.fdigits = fdig
	endelse
	pcwi_print_info,ppar,pre,'number of digits in image numbers', ppar.fdigits
	;
	; create file spec
	...
	pcfg = pcwi_read_cfgs(indir,filespec=fspec)
	nf = n_elements(pcfg)
	pcwi_print_info,ppar,pre,'Number of raw input images',nf
	;
	; write out a complete listing
	pcwi_print_cfgs,pcfg,/silent,/header,outfile=odir+'pcwi.imlog'
	;
	; trim imgtype tag
	pcfg.imgtype = strtrim(pcfg.imgtype,2)
	;
	; trim object name
	pcfg.object = strtrim(pcfg.object,2)
	;
	; exclude biases and test images from process list
	proc = where(strmatch(pcfg.imgtype,'bias') ne 1 and $
		     strmatch(pcfg.imgtype,'test') ne 1 and $
		     strmatch(pcfg.imgtype,'image') ne 1,nproc)
	;
	; if includetest set just exclude biases
	if ppar.includetest eq 1 then $
		proc = where(strmatch(pcfg.imgtype,'bias') ne 1,nproc)
	;
	; is there anything left to process?
	if nproc le 0 then begin
		pcwi_print_info,ppar,pre,'no object/cal images to process', $
			/error
		free_lun,ll
		return
	endif
	...
	;calcfg = pcwi_read_cfgs(caldir,filespec=fspec,count=ncal,/silent)
	;
	; combine them with local calibrations which take precedence
	;if ncal gt 0 then begin
	;	pcwi_print_info,ppar,pre,'Including library calibrations from',caldir
	;	calcfg = [calcfg,pcfg]
	;
	; if nothing found, then we assume that we will use local images
	;endif else begin
		pcwi_print_info,ppar,pre,'Using only local calibrations'
		calcfg = pcfg
		ncal = nf
	;endelse
	pcwi_print_info,ppar,pre,'Number of images in calibration pool',ncal,$
		format='(a,i5)'
	;
	...
	if nprofs gt 0 then begin
		...
		ppar.profs = rlst
	endif else pcwi_print_info,ppar,pre,'no slice profile observations found',/warning
	;
	...
	if ntrrs gt 0 or ndrrs gt 0 then begin
		...
		nrrs = 0
		pcwi_print_info,ppar,pre,'no relative response images found',/warning
	endelse
	;
	; find sky observation images
	skys = where(pcfg.skyobs eq 1, nskys)
	if nskys gt 0 then begin
		ppar.skyexists = 1
		ppar.nskys = nskys
		rangepar,pcfg[skys].imgnum,rlst
		ppar.skys = rlst
	endif else pcwi_print_info,ppar,pre,'no sky observations found',/warning
	;
	; find standard star observation images
	stds = pcwi_find_stds(pcfg,ppar,nstds)
	if nstds le 0 then $
		pcwi_print_info,ppar,pre,'no standard star images found',/warning
	;
	; get image numbers to be processed
	imnums = pcfg[proc].imgnum
	rangepar,imnums,imlist
	ppar.imnum = imlist
	ppar.npims = nproc
	;
	; report
	pcwi_print_info,ppar,pre,'processing '+strtrim(strn(nproc),2)+' images'
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	; GROUP BIASES
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	pcwi_group_biases,calcfg,ppar,bcfg
	;
	; do we have any bias groups?
	if ppar.nbgrps le 0 then $
		pcwi_print_info,ppar,pre,'no bias groups found',/warning
	pcwi_print_info,ppar,pre,'Number of bias groups', ppar.nbgrps
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	; GROUP DARKS
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	pcwi_group_darks,calcfg,ppar,dcfg
	;
	; do we have any dark groups?
	if ppar.ndgrps le 0 then $
		pcwi_print_info,ppar,pre,'no dark groups found',/warning
	pcwi_print_info,ppar,pre,'Number of dark groups', ppar.ndgrps
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	; GROUP FLATS
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	pcwi_group_flats,calcfg,ppar,fcfg
	;
	; do we have any flat groups?
	if ppar.nfgrps le 0 then $
		pcwi_print_info,ppar,pre,'no flat groups found',/warning
	pcwi_print_info,ppar,pre,'Number of flat groups', ppar.nfgrps
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	; GROUP CBARS AND ARC FILES (GEOM)
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	pcwi_group_geom,calcfg,ppar,ccfg,acfg,ngeom
	if ppar.ncbars le 0 then $
		pcwi_print_info,ppar,pre,'no geom groups found',/warning
	pcwi_print_info,ppar,pre,'Number of geom groups', ppar.ncbars
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	if nprofs gt 0 then $
		cpcfg = calcfg[profs]
	pcwi_print_info,ppar,pre,'Number of slice profile images',nprofs
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	if nskys gt 0 then $
		scfg = pcfg[skys]
	pcwi_print_info,ppar,pre,'Number of sky images',nskys
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	if ndrrs gt 0 then $
		rdcfg = calcfg[drrs]
	pcwi_print_info,ppar,pre,'Number of relative response images',nrrs
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	if nstds gt 0 then $
		stdcfg = pcfg[stds]
	pcwi_print_info,ppar,pre,'Number of standard star images',nstds
	;
	; write out master PCWI_PPAR into file
	ppar.ppfname = 'pcwi.ppar'
	pcwi_write_ppar,ppar,/archive
	;
	; set up configuration matching: here is our list of 
	; default tags to match in the PCWI_CFG struct
	...
	filestamp,odir+'pcwi.link',/arch
	openw,kl,odir+'pcwi.link',/get_lun
	...
	for i=0,nproc-1 do begin
		...
		;
		; get image summary
		pcwi_print_cfgs,pcfg[p],imsum,/silent
		;
		; format for output
		...
		if ppar.nbgrps gt 0 then begin
			mcfg = pcwi_associate(bcfg,pcfg[p],ppar,count=b)
			if b eq 1 then begin
				mbfile = mcfg.groupfile
				blink = mcfg.groupnum
				;
				; log
				pcwi_print_info,ppar,pre,'master bias file = '+$
					mbfile
			;
		...
		if ppar.ndgrps gt 0 then begin
			mcfg = pcwi_associate(dcfg,pcfg[p],ppar,count=d)
			if d eq 1 then begin
				mdfile = mcfg.groupfile
				dlink = mcfg.groupnum
				;
				; log
				pcwi_print_info,ppar,pre,'master dark file = '+$
					mdfile
			;
		...
		if strmatch(pcfg[p].imgtype,'dark') ne 1 and ppar.nfgrps gt 0 then begin
			mcfg = pcwi_match_cfg(fcfg,pcfg[p],ppar,mtags,imgtype='cflat',/time,count=f,/silent,cwi=cwi)
			if f eq 1 then begin
				mffile = mcfg.groupfile
				flink = mcfg.groupnum
				;
				; log
				pcwi_print_info,ppar,pre,'master flat file = ' + mffile
			;
		...
		if strmatch(pcfg[p].imgtype,'dark') ne 1 and ppar.ncbars gt 0 and ppar.narcs gt 0 then begin
			mcfg = pcwi_match_cfg(ccfg,pcfg[p],ppar,mtags,imgtype='cbars',/time,count=c,/silent,cwi=cwi)
			if c eq 1 then begin
				...
				;
				; log
				pcwi_print_info,ppar,pre,'cbars file = '+cbfile
				pcwi_print_info,ppar,pre,'arc   file = '+arfile
				;
		...
		if strmatch(pcfg[p].imgtype,'dark') ne 1 and ppar.nprofs gt 0 and $
			links[icbar] ge 0 and links[iarc] ge 0 then begin
			mcfg = pcwi_match_cfg(cpcfg,pcfg[p],ppar,mtags,count=s,/time,cwi=cwi)
			if s eq 1 then begin
				...
				;
				; log
				pcwi_print_info,ppar,pre,'slice profile file = '+pfile
				;
		...
		if strmatch(pcfg[p].imgtype,'object') eq 1 and $
		   pcfg[p].skyobs eq 0 and ppar.nskys gt 0 then begin
			mcfg = pcwi_match_cfg(scfg,pcfg[p],ppar,mtags,/object,/time,count=s,/silent,cwi=cwi)
			if s eq 1 then begin
				;
				; record sky filename
				...
				;
				; log
				pcwi_print_info,ppar,pre,'sky file = '+skfile
			endif else begin
				pcwi_print_info,ppar,pre,'No sky obs with same object name', $
					pcfg[p].object,format='(a,2x,a)',/warning
				slink = -1
			endelse
			;
			; set sky link
		...
		; also require geometry solution
		if strmatch(pcfg[p].imgtype,'dark') ne 1 and ppar.nrrs gt 0 and $
			links[icbar] ge 0 and links[iarc] ge 0 then begin
			;
			; twilight flats
			if ntrrs gt 0 then $
				mtcfg = pcwi_match_cfg(rtcfg,pcfg[p],ppar,mtags,count=rt,/time) $
			else	rt = 0
			;
			; dome flats
			if ndrrs gt 0 then $
				mdcfg = pcwi_match_cfg(rdcfg,pcfg[p],ppar,mtags,count=rd,/time) $
			else	rd = 0
			;
			; do we have a choice?
			...
			if r eq 1 then begin
				...
				;
				; log
				pcwi_print_info,ppar,pre,'relative response file = '+rrfile
			endif else begin
				rlink = -1
			endelse
			;
		...
		if nstds gt 0 and strmatch(pcfg[p].imgtype,'object') eq 1 and links[irrsp] ge 0 then begin
			mcfg = pcwi_match_cfg(stdcfg,pcfg[p],ppar,mtags,count=std,/time)
			if std eq 1 then begin
				...
				;
				; log
				pcwi_print_info,ppar,pre,'standard star observation file = '+stdfile
			endif else begin
				stdlink = -1
			endelse
			;
		...
		;
		; write out links
		printf,kl,pcfg[p].imgnum,links,imsum,format='(10i9,2x,a)'
