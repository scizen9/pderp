; $Id: pcwi_stage3flat.pro | Wed Mar 4 12:02:01 2015 -0800 | Don Neill  $
;
...
;
; PURPOSE:
;	This procedure takes the output from PCWI_STAGE1 and applies further
...
;
; CALLING SEQUENCE:
;	PCWI_STAGE3FLAT, Pparfname, Linkfname
;
; OPTIONAL INPUTS:
...
;
;	PCWI_STAGE3FLAT,'night1/redux/pcwi.ppar'
;
...
;-
pro pcwi_stage3flat,ppfname,linkfname,help=help,select=select, $
	...
	if keyword_set(proc_imgnums) then begin
		...
		if n_elements(proc_flatnums) ne nproc then begin
			pcwi_print_info,ppar,pre,'Number of flats must equal number of images',/error
			return
		endif
		imgnum = proc_imgnums
		fnums = proc_flatnums
	...
	; if not use link file
		...
		;
		; read link file
		pcwi_read_links,ppar,linkfname,imgnum,flat=fnums,count=nproc, $
			select=select
	...
	;
	; log file
	lgfil = reddir + 'pcwi_stage3flat.log'
	...
	for i=0,nproc-1 do begin
		...
		if file_test(obfil) then begin
			...
			;
			; final output file
			ofil = pcwi_get_imname(ppar,imgnum[i],'_intf',/reduced)
			;
			; trim image type
			...
			if ppar.clobber eq 1 or not file_test(ofil) then begin
				...
				if fnums[i] ge 0 then begin
					...
					if file_test(mfppfn) then begin
						...
						;
						; log that we got it
						pcwi_print_info,ppar,pre,'flat file = '+mffile
					endif else begin
						;
						; log that we haven't got it
						pcwi_print_info,ppar,pre,'flat file not found: '+mffile,/error
					endelse
				endif
				;
				...
				if do_flat then begin
					;
					; skip flat correction for continuum bars
					if strpos(pcfg.imgtype,'bars') ge 0 then begin
						pcwi_print_info,ppar,pre,'skipping flattening of cbars image',/info

					;
					; do the flat for everyone else
					endif else begin
						;
						...
						ofil = pcwi_get_imname(ppar,imgnum[i],'_mskf',/nodir)
						...
						ofil = pcwi_get_imname(ppar,imgnum[i],'_varf',/nodir)
						...
						;
						; write out final intensity image
						ofil = pcwi_get_imname(ppar,imgnum[i],'_intf',/nodir)
						pcwi_write_image,img,hdr,ofil,ppar
					endelse
					;
					; handle the case when no flat frames were taken
				endif else begin
					pcwi_print_info,ppar,pre,'cannot associate with any master flat: '+ $
