; $Id: pcwi_slice_rr.pro | Wed Mar 4 12:02:01 2015 -0800 | Don Neill  $
;
...
;	PCWI_CFG struct, then the output rr image would have the
...
;-
pro pcwi_slice_rr,pcfg,ppar,rr
	;
	; setup
	pre = 'PCWI_SLICE_RR'
	;
	...
	if strmatch(strtrim(pcfg.imgtype,2),'dflat') eq 0 and $
	   strmatch(strtrim(pcfg.imgtype,2),'tflat') eq 0 then begin
		pcwi_print_info,ppar,pre,'not an rr obs',/warning
	endif
	;
	; directories
	...
	ofil = pcwi_get_imname(ppar,pcfg.imgnum,'_rr',/reduced)
	...
	icub = pcwi_read_image(pcfg.imgnum,ppar,'_icubep',hdr,/calib,status=stat)
	;
	...
	;
	; log results
	pcwi_print_info,ppar,pre,'Resp. Pars: X0, X1, Y0, Y1, Wav0, Wav1', $
	...
	if doplots ge 2 then begin
		...
		plot,w[y0:y1],refr,xthick=th,ythick=th,charsi=si,charthi=th, $
			title='Img: '+strn(pcfg.imgnum)+' Reference Slice: '+strn(irs), $
	...
	for i=0,23 do begin
		...
		;
		; log results
		pcwi_print_info,ppar,pre,'Resp. Fit: Slice#, Coefs',i,fco, $
		...
		if doplots ge 2 then begin
			...
			plot,w[y0:y1],nrr,xthick=th,ythick=th,charsi=si, $
				charthi=th, $
				title='Img: '+strn(pcfg.imgnum)+' Slice: '+strn(i)+rlab, $
	...
	sxaddpar,hdr,'CNAME1','PCWI DEC',' DEC name'
	...
	sxaddpar,hdr,'CNAME2','PCWI RR Wavelength',' Wavelength name'
	...
	ofil = pcwi_get_imname(ppar,pcfg.imgnum,'_rr',/nodir)
	pcwi_write_image,rrs,hdr,ofil,ppar
	return
end
