; $Id: pcwi_stage2dark.pro | Wed Mar 4 12:02:01 2015 -0800 | Don Neill  $
;
...
;
; PURPOSE:
;	This procedure takes the output from PCWI_STAGE1 and subtracts the
;	master dark frame.
;
; CATEGORY:
...
;
; CALLING SEQUENCE:
;	PCWI_STAGE2DARK, Pparfname, Linkfname
;
; OPTIONAL INPUTS:
...
;
;	PCWI_STAGE2DARK,'night1/redux/pcwi.ppar'
;
...
;-
pro pcwi_stage2dark,ppfname,linkfname,help=help,select=select, $
	...
	if keyword_set(proc_imgnums) then begin
		...
		if n_elements(proc_darknums) ne nproc then begin
			pcwi_print_info,ppar,pre,'Number of darks must equal number of images',/error
			return
		endif
		imgnum = proc_imgnums
		dnums = proc_darknums
	...
	; if not use link file
		...
		;
		; read link file
		pcwi_read_links,ppar,linkfname,imgnum,dark=dnums,count=nproc, $
			select=select
	...
	;
	; log file
	lgfil = reddir + 'pcwi_stage2dark.log'
	...
	for i=0,nproc-1 do begin
		...
		if file_test(obfil) then begin
			...
			;
			; final output file
			ofil = pcwi_get_imname(ppar,imgnum[i],'_intd',/reduced)
			;
			; trim image type
			...
			if ppar.clobber eq 1 or not file_test(ofil) then begin
				...
				vfil = pcwi_get_imname(ppar,imgnum[i],'_var',/reduced)
				if file_test(vfil) then begin
					...
					pcwi_print_info,ppar,pre, $
					    ...
					    obfil,/warning
				endelse
				mfil = pcwi_get_imname(ppar,imgnum[i],'_msk',/reduced)
				...
				if dnums[i] ge 0 then begin
					...
					if file_test(mdppfn) then begin
						...
						;
						; log that we got it
						pcwi_print_info,ppar,pre,'dark file = '+mdfile
					endif else begin
						;
						; log that we haven't got it
						pcwi_print_info,ppar,pre,'dark file not found: '+mdfile,/error
					endelse
				endif
				;
				...
				if do_dark then begin
					...
					else	pcwi_print_info,ppar,pre,'unable to scale dark by exposure time',/warning
					;
					; do subtraction
					img = img - mdark*fac
					;
					...
					ofil = pcwi_get_imname(ppar,imgnum[i],'_mskd',/nodir)
					...
					ofil = pcwi_get_imname(ppar,imgnum[i],'_vard',/nodir)
					...
					ofil = pcwi_get_imname(ppar,imgnum[i],'_intd',/nodir)
					pcwi_write_image,img,hdr,ofil,ppar
					;
					; handle the case when no dark frames were taken
				endif else begin
					pcwi_print_info,ppar,pre,'cannot associate with any master dark: '+ $
			...
			; end check if output file exists already
				...
				if ppar.clobber eq 0 and file_test(ofil) then $
					pcwi_print_info,ppar,pre,'processed file exists already: '+ofil,/warning
			endelse
		;
