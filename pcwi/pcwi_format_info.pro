; $Id: pcwi_format_info.pro | Tue Mar 3 16:45:54 2015 -0800 | Don Neill  $
;
...
;
; PURPOSE:
;	Create header modification text file for input to PCWI_FIX_HEADERS
...
;	information required for PCWI_FIX_HEADERS.PRO.
;
; PROCEDURE:
...
;	needed by PCWI_FIX_HEADERS.PRO.
;
...
;-
pro pcwi_format_info,froot=froot,fdigits=fdigits,update=update
;
; setup
pre = 'PCWI_FORMAT_INFO'
version = repstr('$Revision: ca58c78 $ $Date: Tue Mar 3 16:45:54 2015 -0800 $','$','')
q = ''
;
; check allhdr.txt
afile = !PCWI_DATA + 'allhdr.txt'
if not keyword_set(update) and not file_test(afile) then begin
	print,pre+': ERROR - allhdr.txt not found, looking here: ',!PCWI_DATA
	print,pre+': Fix !PCWI_DATA in startup.pro and rerun.'
	return
endif
;
; get input file spec
...
;
; Keyword processing info (see PCWI_FIX_HEADERS.PRO for details)
	...
	if n_elements(hdr) gt 1 then begin
		...
		if strmatch(val,'test') eq 0 and strmatch(val,'bias') eq 0 then begin
			if abs(gratpos) gt 50000L then begin
				...
				cwave = pcwi_cwave_medrez(campos,gratpos)
			endif else begin
				...
				;
				; calculate central wavelength
				cwave = pcwi_central_wave(grat,campos,gratpos)
			endelse
			;
			; calculate grating anomoly
			gratanom = pcwi_gratanom(grat,gratpos,cwave)
			;
