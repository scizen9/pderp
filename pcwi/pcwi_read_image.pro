; $Id: pcwi_read_image.pro | Wed Mar 4 12:02:01 2015 -0800 | Don Neill  $
;
...
;-
function pcwi_read_image,imgnum,ppar,tail,hdr, $
	...
	if n_params(0) lt 3 then begin
		if not keyword_set(calib) then begin
			pcwi_print_info,ppar,pre,'use pcwi_read_raw to read raw images',/error
			return,-1
		endif else tail = ''
	endif
	;
	...
	if keyword_set(calib) then $
		idir = pcwi_expand_dir(ppar.caldir) $
	else	idir = pcwi_expand_dir(ppar.reddir)
	if not file_test(idir,/directory,/executable,/read) then begin
		pcwi_print_info,ppar,pre,'input directory not accessible',idir,/error
		status = 2
		return,-1
	endif
	;
	...
	if file_test(ifil) then begin
		...
		if strpos(tail,'msk') ge 0 then begin
			...
			pcwi_print_info,ppar,pre,'read mask file with no scaling',ifil,format='(a,a)'
		;
		; everything else should be floats
		endif else begin
			img = mrdfits(ifil,0,hdr,/silent,/fscale)
			pcwi_print_info,ppar,pre,'read image file with /fscale',ifil,format='(a,a)'
		endelse
		status = 0
	;
	; report non-existence
	endif else begin
		pcwi_print_info,ppar,pre,'file not found',ifil,/error
		status = 1
		img = -1
	endelse
	;
