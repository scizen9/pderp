; $Id: pcwi_quick.pro | Thu Apr 23 11:31:25 2015 -0700 | Don Neill  $
;
...
;
; CALLING SEQUENCE:
;	PCWI_QUICK, RawDir, ReducedDir, CalibDir, DataDir
;
; OPTIONAL INPUTS:
...
;	for running PCWI_STAGE{N}.  Master calibration image pipeline parameter 
...
;-
pro pcwi_quick,rawdir,reduceddir,calibdir,datadir, $
	imgtarg=imgtarg, $
	froot=froot, $
	fdigits=fdigits, $
	...
	if keyword_set(imgtarg) then begin
		it = where(pcfg.imgnum eq imgtarg, nit)
		if nit le 0 then begin
			pcwi_print_info,ppar,pre,'Requested image number not found',imgtarg,/error
			return
		endif
		if strpos(pcfg[it].imgtype,'object') lt 0 and $
		   strpos(pcfg[it].imgtype,'dflat') lt 0 then $
			pcwi_print_info,ppar,pre,'Requested image not an object/dflat image',imgtarg,pcfg[it].imgtype,/warn
	;
	; or just get most recent object image
	endif else begin
		ob = where(strpos(pcfg.imgtype,'object') ge 0 or $
			   strpos(pcfg.imgtype,'dflat') ge 0, nob)
		if nob le 0 then begin
			pcwi_print_info,ppar,pre,'No object/dflat images taken yet',/error
			return
		endif
		it = where(pcfg[ob].juliandate eq max(pcfg[ob].juliandate))
		it = [ob[it]]
	endelse
	;
	; check other params
	...
	;
	; log file
	lgfil = odir + 'pcwi_quick_'+strn(pcfg[it].imgnum)+'.log'
	...
	printf,ll,'Target Img: '+strn(pcfg[it].imgnum)
	...
	pcwi_print_info,ppar,pre,'Number of input images',nf
	;
	; trim imgtype tag
	...
	if nrrs gt 0 then begin
		...
		ppar.rrs = rlst
	endif else pcwi_print_info,ppar,pre,'no relative response images found',/warning
	;
	...
	pcwi_print_info,ppar,pre,'number of bias groups = '+ $
	...
	pcwi_print_info,ppar,pre,'number of dark groups = '+ $
	...
	pcwi_print_info,ppar,pre,'number of flat groups = '+ $
	...
	ppar.ncbars = ngeom
	ppar.narcs = ngeom
	pcwi_print_info,ppar,pre,'number of cbars, arc images',ngeom
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	for i=0,nproc-1 do begin
		...
		if strmatch(pcfg[p].imgtype,'dark') ne 1 and ppar.nfgrps gt 0 then begin
			mcfg = pcwi_match_cfg(fcfg,pcfg[p],ppar,mtags,imgtype='cflat',/time,count=f,/silent)
		...
		if strmatch(pcfg[p].imgtype,'dark') ne 1 and ppar.ncbars gt 0 and ppar.narcs gt 0 then begin
			mcfg = pcwi_match_cfg(ccfg,pcfg[p],ppar,mtags,imgtype='cbars',/time,count=c,/silent)
		...
		if strmatch(pcfg[p].imgtype,'dark') ne 1 and ppar.nprofs gt 0 and $
			...
			mcfg = pcwi_match_cfg(pcfg,pcfg[p],ppar,mtags,count=s,/time)
		...
		if strmatch(pcfg[p].imgtype,'object') eq 1 and $
		   pcfg[p].skyobs eq 0 and ppar.nskys gt 0 then begin
			mcfg = pcwi_match_cfg(scfg,pcfg[p],ppar,mtags,/object,/time,count=s,/silent)
			if s eq 1 then begin
				;
				; record sky filename
		...
		if strmatch(pcfg[p].imgtype,'dark') ne 1 and ppar.nrrs gt 0 and $
			links[icbar] ge 0 and links[iarc] ge 0 then begin
			mcfg = pcwi_match_cfg(rcfg,pcfg[p],ppar,mtags,count=r,/time)
	...
	pcwi_read_links,ppar,linkfname,inums,bias=bnums,dark=dnums,flat=fnums, $
		cbar=cnums,arc=anums,prof=pnums,rrsp=rnums
	;
	; target image pointer
	ti = (where(inums eq pcfg[it].imgnum, nti))[0]
	if nti le 0 then begin
		pcwi_print_info,ppar,pre,'Target image not found in link file',/error
		return
	endif
	;
	...
	if dnums[ti] ge 0 then begin
		dpar = pcwi_read_ppar(odir+'mdark_'+strn(dnums[ti])+'.ppar')
	...
	if fnums[ti] ge 0 then begin
		fpar = pcwi_read_ppar(odir+'mflat_'+strn(fnums[ti])+'.ppar')
	...
	;
	; do stage 1
	pcwi_stage1,proc_imgnums=s1img,proc_biasnums=s1bias
	stagetail[1] = 'int.fits'
	;
	; do stage 2 dark
	if dnums[ti] ge 0 then begin
		pcwi_stage2dark,proc_imgnums=s1img,proc_darknums=s2dark
		stagetail[2] = 'intd.fits'
	endif
	;
	; do stage 3 flat
	if fnums[ti] ge 0 then begin
		pcwi_stage3flat,proc_imgnums=s1img,proc_flatnums=s3flat
		stagetail[3] = 'intf.fits'
	endif
	;
	; do stage 4 geom
	if cnums[ti] ge 0 and anums[ti] ge 0 then begin
		pcwi_stage4geom,proc_imgnums=s1img,proc_cbarnums=s4cbars,proc_arcnums=s4arcs
		stagetail[4] = 'icube.fits'
	endif
	;
	; do stage 5 profile
	if pnums[ti] ge 0 then begin
		pcwi_stage5prof,proc_imgnums=s1img,proc_profnums=s5profs
	...
	if pnums[ti] ge 0 and rnums[ti] ge 0 then begin
		pcwi_stage6rr,proc_imgnums=s1img,proc_rrnums=s6rrs
	...
	if lstage ge 4 then begin
		...
		zrat = pgeom.slscl/(pgeom.pxscl*pgeom.xbinsize)
		;
		; pick correct tail
	...
	if keyword_set(ds9) then begin
		pcwi_print_info,ppar,pre,'Displaying... ',ds9cmd, $
			format='(a,a)'
		spawn,ds9cmd
	endif
	;
