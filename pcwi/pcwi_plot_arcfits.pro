; $Id: pcwi_plot_arcfits.pro | Fri Apr 24 12:15:57 2015 -0700 | Don Neill  $
;
...
;
; CALLING SEQUENCE:
;	PCWI_PLOT_ARCFITS, Specs, Pgeom, Ppar, CntCoeff, FinCoeff, Sigmas
;
; INPUTS:
...
;-
;
pro pcwi_plot_arcfits, specs, pgeom, ppar, cntcoeff, fincoeff, sigmas, $
	fwaves, dwaves, $
	tweak=tweak, plot_file=plot_file

pre = 'PCWI_PLOT_ARCFITS'
q=''
;
...
imglab = 'Img # '+strn(imgnum)+' Fl: '+strtrim(pgeom.filter,2)+' Gr: '+strtrim(pgeom.gratid,2)
;
; is the N+S mask in?
...
if not keyword_set(plot_file) then $
	window,1,title='pcwi_plot_arcfits'
;
; set up plots
!p.multi=0
...
wavall0 = pgeom.waveall0
wavall1 = pgeom.waveall1
wavgood0 = pgeom.wavegood0
wavgood1 = pgeom.wavegood1
wavmid = pgeom.wavemid
cwave = pgeom.cwave
minwav = pgeom.wave0out
maxwav = pgeom.wave1out
;
; atlas name
atnam = pgeom.refname
;
...
for b=0,119 do begin
	...
	if display or keyword_set(plot_file) then begin
		...
		pcwi_legend,[atnam+' Atlas',fittype+' Arc Fit'],linesty=[0,0],charthi=th, $
		...
		if keyword_set(tweak) and display then begin
			...
			pcwi_legend,['All','Good','CWAVE'],linesty=[1,1,1],charthi=th, $
