...
;	clr_color = color for background of box when cleared
...
;	Added clr_color D. Neill Sep. 2007
;-
pro pcwi_legend, items, BOTTOM_LEGEND=bottom, BOX = box, CENTER_LEGEND=center, $
    ...
    FILL=filli, HELP = help, HORIZONTAL=horizontal,LEFT_LEGEND=left, $
    ...
    OUTLINE_COLOR = outline_color,CLR_COLOR = clr_color
;
;       =====>> HELP
;
on_error,2
if keyword_set(help) then begin & doc_library,'pcwi_legend' & return & endif
;
...
if n eq 0 then message,'No inputs!  For help, type pcwi_legend,/help.'
if ni eq 0 then begin
  items = replicate('',n)                               ; DEFAULT BLANK ARRAY
endif else begin
  if size(items,/TNAME) NE 'STRING' then message, $
      'First parameter must be a string array.  For help, type pcwi_legend,/help.'
  if ni ne n then message,'Must have number of items equal to '+strn
endelse
symline = (np ne 0) or (nl ne 0)                        ; FLAG TO PLOT SYM/LINE
 if (nf ne 0) and (nf ne n) and (nf NE 1) then message, $
        'Must have 0, 1 or '+strn+' elements in FILL array.'
 ...
 case nf of 
 0: fill = intarr(n)+1              ;Default = solid
 1: fill = intarr(n)  + filli
 else: fill = filli
 endcase 
...
case n_elements(position) of
 ...
 else: message,'Position keyword can have 0, 1, or 2 elements only. Try pcwi_legend,/help.'
endcase
...
for iclr = 0,clear do begin
 ...
 for i = ii[0],ii[1],ii[2] do begin
  ...
  if (psym[i] eq 8) and (N_elements(usersym) GT 1) then $
	  if fill[i] gt 0 then $
                usersym,usersym,/fill,color=colors[i] $
	  else  usersym,usersym,color=colors[i]
...
;       =====>> CLEAR AREA
	...
	if n_elements(clr_color) le 0 then clr_color=-1
        polyfill,[x,xend,xend,x,x],y + [0,0,ywidth,ywidth,0],/norm,color=clr_color
