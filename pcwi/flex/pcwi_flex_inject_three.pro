...
pro pcwi_flex_inject_three,ppar,kgeom,imgnum
  flex_counter = 0;
  ; loop over all files to be processed.
  for f=0,n_elements(imgnum)-1 do begin
     message,"Image being processed: "+string(imgnum[f]),/info
     flexpar_fname = pcwi_get_imname(ppar, imgnum[f],"_flexpar",/reduced);
     if file_test(flexpar_fname) then begin
      ...
      ; check if this is a nod-and-shuffle observation
              cub_reference_fname =  pcwi_get_imname(ppar,flexpar.reference,"_scube",/reduced)
              if file_test(cub_reference_fname) then begin
                 cub_template_fname = pcwi_get_imname(ppar,flexpar.template,"_scube",/reduced)
                 flex_ns = 1
              endif else begin
                 cub_reference_fname =  pcwi_get_imname(ppar,flexpar.reference,"_icube",/reduced)
                 cub_template_fname = pcwi_get_imname(ppar,flexpar.template,"_icube",/reduced)
                 flex_ns = 0 
...
;                 correl_optimize,sli_ref,sli_tmp,xoff,yoff,magnify=10
                 ...
                 sli_ref_med = pcwi_median(sli_ref,[1,15])
                 sli_tmp_med = pcwi_median(sli_tmp,[1,15])                
                 pcwi_flex_collapse, sli_ref, prof_ref, spec_ref_dummy
                 pcwi_flex_collapse, sli_tmp, prof_tmp, spec_tmp_dummy
                 sli_ref -= sli_ref_med
                 sli_tmp -= sli_tmp_med
                 pcwi_flex_collapse, sli_ref, prof_ref_dummy, spec_ref, /continuum
                 pcwi_flex_collapse, sli_tmp, prof_tmp_dummy, spec_tmp, /continuum
                 ...
                 if flexpar.refmask eq 1 then begin
                    ...
                    if masksize[1] eq 2 then $
                       pcwi_flex_apply_mask,spec_ref,spec_tmp,wvs,refmask
                 ...
                 ppar.display=1
                 pcwi_xspec, spec_ref, spec_tmp, ppar, spec_offset, spec_max, /plot,/pad,/shift
                 ppar.display=1
                 pcwi_xspec, prof_ref, prof_tmp, ppar, prof_offset, prof_max, /plot,/pad,/shift
...
;              stop
              ...
              cub_fname =  pcwi_get_imname(ppar,flexpar.imno,"_icube",/reduced)
              if file_test(cub_fname) then file_delete,cub_fname ;
              cub_fname =  pcwi_get_imname(ppar,flexpar.imno,"_mcube",/reduced)
              if file_test(cub_fname) then file_delete,cub_fname ;
              cub_fname =  pcwi_get_imname(ppar,flexpar.imno,"_vcube",/reduced)
              if file_test(cub_fname) then file_delete,cub_fname ;
              if flex_ns then begin
              cub_fname =  pcwi_get_imname(ppar,flexpar.imno,"_scube",/reduced)
              if file_test(cub_fname) then file_delete,cub_fname ;
              cub_fname =  pcwi_get_imname(ppar,flexpar.imno,"_ocube",/reduced)
              if file_test(cub_fname) then file_delete,cub_fname ;
              endif;
           endif                                                  ; computed eq 0 
        endif; method is 2.                                                     ; 
     ...
     if flex_counter gt 0 then begin
        ...
        message,"Re-run pcwi_stage4geom",/info
     endif                      ;
end; pcwi_flex_inject_three
