; PCWI_FLEX_INJECT_ONE,ppar,kgeom,flexpar
;
...
;flexure geometry will be used when pcwi_apply_geom is called. 
;                                                      
pro pcwi_flex_inject_one,ppar, kgeom, flexpar, flexparmask
;
...
if flexpar.method eq 2 then begin 
   ...
   flexpar_fname = pcwi_get_imname(ppar, flexpar.imno,"_flexpar",/reduced)
   ...
   if flexpar.computed eq 0 then begin 
      ...
      template_fname = pcwi_get_imname(ppar,flexpar.template,"_sky",/reduced)
      if file_test(template_fname) then begin
         reference_fname = pcwi_get_imname(ppar,flexpar.reference,"_sky",/reduced)
         ...
         template_fname = pcwi_get_imname(ppar,flexpar.template,"_int",/reduced)
         reference_fname = pcwi_get_imname(ppar,flexpar.reference,"_int",/reduced)
         img_template = mrdfits(template_fname,0,hdr_template,/silent,/fscale)
         img_reference = mrdfits(reference_fname,0,hdr_reference,/silent,/fscale)
         flex_ns = 0 
      endelse ; _sky exist? 
      ...
      if flexpar.refmask eq 1 then begin
         refmask_fname = pcwi_get_imname(ppar,flexpar.reference,"_refmask",/reduced);
         if file_test(refmask_fname) then begin
            flex_ref_mask = mrdfits(refmask_fname);
         endif else begin
            ; we need to have a reverse map for this. 
            wavemap_fname = pcwi_get_imname(ppar,kgeom.cbarsimgnum,"_wavemap",/reduced);
            if not file_test(wavemap_fname) then pcwi_reverse_geom,kgeom,ppar
      ...
      mask_fname = pcwi_get_imname(ppar,flexpar.imno,"_flexmask",/reduced)
      ...
      flex_ref_median = pcwi_median(img_reference,[1,15])
      flex_tmp_median = pcwi_median(img_template,[1,15])
...
; shifts and rotations are allowed.
      ...
      pcwi_warp_coeffs, xi, yi, fx,fy, xo, yo
      ;                                
