...
; corrected. 
;
; Method: see pcwi_flex.readme; defaults to 1.
; 
...
pro pcwi_flex_prep, ppfname, method=method, ref=ref, imno=imno, tmpno=tmpno, skipx=skipx , wavemask=wvmask
...
; check the skipx keyword.
  ...
  if pcwi_verify_ppar(ppar,/init) ne 0 then begin
     ...
     return
                                ...
                                ; directories
  if pcwi_verify_dirs(ppar,indir,odir,cdir,ddir,/nocreate) ne 0 then begin
...
; ok. now we are ready to set this thing up.
  ...
  for f=0,n_elements(imno)-1 do begin
     ...
     strc.kwy_new[*,*,*]=0.000
     strc.skipx = skipx
     strc.reference_fname = pcwi_get_imname(ppar, ref,"_flexref",/reduced)
     strc.template_fname = pcwi_get_imname(ppar, tmpno[f],"_flexcor",/reduced)
     dimmask = size(wvmask)
     if dimmask[1] eq 2 then strc.refmask=1.0 else strc.refmask=0.0
     outfname = pcwi_get_imname(ppar, imno[f],"_flexpar",/reduced)
