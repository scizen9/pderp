; $Id: pcwi_solve_arcs.pro | Wed Apr 29 14:48:41 2015 -0700 | Don Neill  $
;
...
;
; CALLING SEQUENCE:
;	PCWI_SOLVE_ARCS, Specs, Pgeom, Ppar
;
; INPUTS:
...
;-
;
pro pcwi_solve_arcs, specs, pgeom, ppar, tweak=tweak, plot_file=plot_file
...
pcwi_fit_center,specs,pgeom,ppar,cntcoeff
;
; check status
if pgeom.status gt 0 then begin
	pcwi_print_info,ppar,pre,'Cannot solve arcs',/error
	return
endif
;
; plot results
if ppar.display ge 1 then begin
	window,0,title='pcwi_solve_arcs'
	!p.multi=[0,1,2]
	si = 1.5
...
pcwi_read_atlas,pgeom,ppar,refspec,refwvl,refdisp
;
...
if keyword_set(tweak) then begin
	...
	if niter eq 1 then $
		pcwi_print_info,ppar,pre,'using '+strn(niter)+' iteration to measure RMS of initial central fit' $
	else	pcwi_print_info,ppar,pre,'using '+strn(niter)+' iterations to tweak higher order terms'
	;
	pcwi_print_info,ppar,pre,'Spectral line finding/matching thresh (frac. of res.): PkDel', $
	...
	if ppar.display ge 3 then begin
		...
		pcwi_legend,['PeakFit','WavRng'],linesty=[0,5],thick=[1.0,th], $
	...
	if ddisplay then $
		window,1,title='pcwi_solve_arcs(2)'
	;
	...
	for iter = 0,niter-1 do begin
		...
		for b=0, 119 do begin
			...
			if nmatchedpeaks le 2 then begin
				print,' '
				pcwi_print_info,ppar,pre,'Not enough peaks matched in Bar # '+strn(b)+': '+strn(nmatchedpeaks),/warning
				barstat[b]=9
				goto, errbar
			endif
			;
			...
			while nbad gt 0 do begin
				...
				if nmatchedpeaks le 0 then begin
					pcwi_print_info,ppar,pre,'All points rejected',/error
					barstat[b]=10
					goto, errbar
				endif
				;
			...
			if ddisplay and iter eq 0 then begin
				pcwi_legend,['Good','NoAtlas','Reject'],linesty=[0,0,0], $
		...
		pcwi_print_info,ppar,pre,"Iteration "+string(iter+1,"(i2)")+" of "+ $
		...
		if ppar.cleancoeffs and niter gt 1 then begin
			...
			if ppar.display ge 3 and clnplot then $
				window,2,title='pcwi_clean_coeffs'
...
if keyword_set(tweak) then begin
	...
	for b=0,119 do begin
		...
		if nmatchedpeaks gt 2 then begin
			...
			;
			; log stats
			pcwi_print_info,ppar,pre,ftype+' Fit: Bar#,Npks,RMS,Cdisp,Coefs', $
			...
			if display then begin
				...
				oplot,!x.crange,-[sigmas[b],sigmas[b]], $
					linesty=1,color=colordex('blue')
				pcwi_legend,["RMS = "+string(sigmas[b],form='(f7.3)')+' Ang', $
			...
			pcwi_print_info,ppar,pre,'Not enough matched peaks for bar',b,nmatchedpeaks,/error
...
if nbargood le 0 then begin
	pcwi_print_info,'No good bar fits',/error
	pgeom.status = 10
	return
endif
;
...
;
; check bar statuses
pgeom.status = max(barstat)
if pgeom.status gt 0 then begin
	pcwi_print_info,ppar,pre,'Some bars had issues, check log for details',/warning
	;
	; if N&S, then this was just for info, carry on
	if pgeom.nasmask then $
		pgeom.status=0
endif
;
; order of the fits
...
;
; log values
pcwi_print_info,ppar,pre,'min,max good wavelengths',pgeom.wavegood0,pgeom.wavegood1, $
	format='(a,2f9.2)'
pcwi_print_info,ppar,pre,'min,max inclusive wavelengths',pgeom.waveall0,pgeom.waveall1, $
	format='(a,2f9.2)'
pcwi_print_info,ppar,pre,'min,max trim wavelengths',pgeom.wave0out,pgeom.wave1out, $
	format='(a,2f9.2)'
pcwi_print_info,ppar,pre,'middle wavelength',pgeom.wavemid,format='(a,f9.2)'
;
; set fit rms values
if keyword_set(tweak) then begin
	...
	pcwi_print_info,ppar,pre,'min/max bar wavelength sigma (Ang)',minmax(sigmas[bargood]), $
		format='(a,f7.3,2x,f7.3)'
	if noutliers gt 0 then $
		pcwi_print_info,ppar,pre,'> 3sig outlier bars present, may want to tweak ppar.pkdel: Imgnum, Bars', $
...
pcwi_plot_arcfits,specs,pgeom,ppar,cntcoeff,fincoeff,sigmas,fwaves,dwaves, $
