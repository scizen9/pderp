; $Id: pcwi_stage5prof.pro | Wed Mar 4 12:02:01 2015 -0800 | Don Neill  $
;
...
;
; PURPOSE:
;	This procedure takes the output from PCWI_STAGE4GEOM and applies 
...
;
; CALLING SEQUENCE:
;	PCWI_STAGE5PROF, Pparfname, Linkfname
;
; OPTIONAL INPUTS:
...
;
;	PCWI_STAGE5PROF,'night1/redux/prof.ppar'
;
...
;-
pro pcwi_stage5prof,ppfname,linkfname,help=help,select=select, $
	...
	if keyword_set(proc_imgnums) then begin
		...
		if n_elements(proc_profnums) ne nproc then begin
			pcwi_print_info,ppar,pre,'Number of profiles must equal number of images',/error
			return
		endif
		imgnum = proc_imgnums
		pnums = proc_profnums
	...
	; if not use link file
		...
		;
		; read link file
		pcwi_read_links,ppar,linkfname,imgnum,prof=pnums,count=nproc, $
			select=select
	...
	;
	; log file
	lgfil = reddir + 'pcwi_stage5prof.log'
	...
	for i=0,nproc-1 do begin
		...
		; image to process
		;
		; require output from pcwi_stage4geom
		...
		if file_test(obfil) then begin
			...
			;
			; final output file
			ofil = pcwi_get_imname(ppar,imgnum[i],'_icubep',/reduced)
			;
			; trim image type
			...
			if ppar.clobber eq 1 or not file_test(ofil) then begin
				...
				if pnums[i] ge 0 then begin
					;
					; get filename
					prf = pcwi_get_imname(ppar,pnums[i],/nodir)
					;
					...
					if file_test(mpfile) then begin
						...
						;
						; log that we got it
						pcwi_print_info,ppar,pre,'slice profile file = '+mpfile
					endif else begin
						;
						; does input prof image exist?
						pinfile = pcwi_get_imname(ppar,pnums[i],'_icube',/reduced)
						if file_test(pinfile) then begin
							do_prof = (1 eq 1)
							pcwi_print_info,ppar,pre,'building slice profile file = '+mpfile
						endif else begin
							;
							; log that we haven't got it
							pcwi_print_info,ppar,pre,'slice profile input file not found: '+pinfile,/warning
						endelse
					endelse
				endif
				;
				...
				if do_prof then begin
					...
					if not file_test(mpfile) then begin
						...
						pcfg = pcwi_read_cfg(pinfile)
						;
						; build master prof
						pcwi_slice_prof,pcfg,ppar
					endif
					;
					; read in master prof
					...
					if sz[0] ne mpsz[0] then $
						pcwi_print_info,ppar,pre,'image/profile size mis-match: img x, prof x', $
					...
					ofil = pcwi_get_imname(ppar,imgnum[i],'_mcubep',/nodir)
					...
					ofil = pcwi_get_imname(ppar,imgnum[i],'_vcubep',/nodir)
					...
					ofil = pcwi_get_imname(ppar,imgnum[i],'_icubep',/nodir)
					...
					if file_test(sfil) then begin
						...
						;
						; write out final intensity image
						ofil = pcwi_get_imname(ppar,imgnum[i],'_scubep',/nodir)
					...
					if file_test(nfil) then begin
						...
						;
						; write out final intensity image
						ofil = pcwi_get_imname(ppar,imgnum[i],'_ocubep',/nodir)
						pcwi_write_image,obj,hdr,ofil,ppar
					endif
					;
					; handle the case when no prof frames were taken
				endif else begin
					pcwi_print_info,ppar,pre,'cannot associate with any master prof: ' + $
