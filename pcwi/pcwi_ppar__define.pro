; $Id: pcwi_ppar__define.pro | Thu Apr 23 11:31:25 2015 -0700 | Don Neill  $
;
...
;	for the PCWI Data Reduction Pipeline.
;
; CATEGORY:
...
;
; SIDE EFFECTS:
;	defines the PCWI_PPAR data structure
;
; PROCEDURE:
;	Provide the automatic structure definition for the PCWI pipeline
;	parameter structure PCWI_PPAR.
;
; EXAMPLE:
;	Generate a Ppfile called 'm82.ppar' for the red side images
;	in directory 'night1' and put it in 'night1/redux':
;
;	PCWI_INIT,'M82.ppar','night1','night1/redux','red*.fit*',ppar=KPP
;
;	KPP will be initialized to have the correct pipeline parameters for
;	the inputs to PCWI_INIT.
;
; NOTES:
...
;-
pro pcwi_ppar__define
;
tmp = { pcwi_ppar, $
;
; output filename
	ppfname:'pcwi.ppar', $	; pipeline parameter set name, e.g. 'm81.ppar'
	lnfname:'pcwi.link', $	; processing links file name, e.g. 'm81.link'
;
...
;
; processing switches
	biasskip1:1, $		; skip first bias frame? (for CWI)
