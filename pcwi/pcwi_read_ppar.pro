; $Id: pcwi_read_ppar.pro | Wed Mar 4 12:02:01 2015 -0800 | Don Neill  $
;
...
;
; CALLING SEQUENCE:
;	Result = PCWI_READ_PPAR( PPAR_FILENAME )
;
; INPUTS:
;	ppfname	- pipeline parameter file (written with PCWI_PPAR_WRITE)
;
; KEYWORDS:
;	VERBOSE - set this to get extra screen output
;
; RETURNS:
;	PCWI DRP parameter struct (as defined in pcwi_ppar__define.pro)
;
; PROCEDURE:
...
;		RAWROOT         '/Users/pcwi/drp/run/14/12/20'
...
;	parameters:
;
;	m82par = PCWI_READ_PPAR('m82.ppar')
;
...
;-
function pcwi_read_ppar,ppfname,verbose=verbose
;
; initialize
	pre = 'PCWI_READ_PPAR'

	A = {pcwi_ppar}		; get blank parameter struct
...
; use defaults, if name not passed in
	if n_params(0) lt 1 or n_elements(ppfname) le 0 then begin
		...
		if not file_test(ppfname,/read) then begin
			...
			if not file_test(ppfname,/read) then begin
				pcwi_print_info,ppar,pre,'default ppar file not found',/error
				return,ppar
			endif
		endif
	endif
