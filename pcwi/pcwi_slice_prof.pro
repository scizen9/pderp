; $Id: pcwi_slice_prof.pro | Wed Mar 4 12:02:01 2015 -0800 | Don Neill  $
;
...
;
; CALLING SEQUENCE:
;	PCWI_SLICE_PROF, Pcfg,  Ppar, Prof
;
; INPUTS:
...
;	PCWI_CFG struct, then the output profile image would have the
...
pro pcwi_slice_prof,pcfg,ppar,profs
	...
	if pcfg.skyobs eq 0 and pcfg.shuffmod eq 0 then begin
		pcwi_print_info,ppar,pre,'not a sky obs',/warning
	endif
	;
	; what kind are we?
	if pcfg.shuffmod eq 1 then $
		pcwi_print_info,ppar,pre,'nod-and-shuffle sky obs',/info
	if pcfg.skyobs eq 1 then $
		pcwi_print_info,ppar,pre,'standard sky obs',/info
	;
	; directories
	...
	ofil = pcwi_get_imname(ppar,pcfg.imgnum,'_prof',/reduced)
	...
	if pcfg.nasmask eq 1 then begin
		if pcfg.shuffmod eq 1 then begin
			;
			; read sky cube from nod-and-shuffle image set
			icub = pcwi_read_image(pcfg.imgnum,ppar,'_scube',hdr,status=stat)
			if stat ne 0 then return
		endif else begin
			;
			; read sky observation image
			icub = pcwi_read_image(pcfg.imgnum,ppar,'_icube',hdr,status=stat)
			if stat ne 0 then return
		endelse
		;
		; get size
		...
		;
		; read sky observation image
		icub = pcwi_read_image(pcfg.imgnum,ppar,'_icube',hdr,/calib,status=stat)
		if stat ne 0 then return
		;
		; get size
		sz = size(icub,/dim)
	...
	;
	; log
	if pcfg.nasmask ne 1 then $
		pcwi_print_info,ppar,pre,':Slice   Bl:Mn    Sg       Gr:Mn    Sg       Rd:Mn    Sg       All:Mn   Sg' $
	else	pcwi_print_info,ppar,pre,':Slice   All:Mn   Sg'
	;
	; display status
	...
	for i=0,23 do begin
		...
		if pcfg.nasmask ne 1 then begin
			;
			...
			mn3 = mo3[0]
			sg3 = sqrt(mo3[1])
			pcwi_print_info,ppar,pre,'',i,mn1,sg1,mn2,sg2,mn3,sg3,mn,sg, $
				format='(a,i4,2x,8f9.4)'
		endif else $
			pcwi_print_info,ppar,pre,'',i,mn,sg, $
		...
		if doplots ge 2 then begin
			...
			plot,prof,xthick=th,ythick=th,charsi=si,charthi=th, $
				title='Image # '+strn(pcfg.imgnum)+', Slice '+strn(i),psym=10, $
				xtitle='Pixel',/xs, $
	...
	ofil = pcwi_get_imname(ppar,pcfg.imgnum,'_prof',/nodir)
	pcwi_write_image,profs,hdr,ofil,ppar
	return
end
