; $Id: pcwi_match_cfg.pro | Tue Mar 3 16:16:17 2015 -0800 | Don Neill  $
;
...
;
; PURPOSE:
;	This function returns the indices within Pcfg of images that
...
;
; CALLING SEQUENCE:
;	Result = PCWI_MATCH_CFG( KCFG, TCFG )
;
; INPUTS:
;	Pcfg	- array of struct PCWI_CFG for a given data set
;	Tcfg	- target struct PCWI_CFG to match
;	Ppar	- pipeline parameter PCWI_PPAR struct
;	Tlist	- list of PCWI_CFG struct tag names to match
;		   Defaults to ['XBINSIZE','YBINSIZE','AMPMODE',
;				'GRATID','GRATPOS','FILTER',
;				'FM4POS','CAMPOS','FOCPOS']
;
; Returns:
;	Pcfg entries that match the target configuration, -1 if none found.
;
; INPUT KEYWORDS:
;	SILENT	- set to suppress warning and error messages (for testing)
;	IMGTYPE	- set to PCWI image type to restrict match to: bias, dark, etc.
...
;	group contained in Pcfg and finds each entry that matches the
...
;-
function pcwi_match_cfg, pcfg, tcfg, ppar, tlist, $
	...
	if pcwi_verify_ppar(ppar,/init,/silent) ne 0 then begin
		pcwi_print_info,ppar,pre,'Ppar error - malformed PCWI_PPAR struct',/error
		return,-1
	endif
	if pcwi_verify_cfg(pcfg,/init,/silent) ne 0 then begin
		pcwi_print_info,ppar,pre,'Search Error - malformed PCWI_CFG struct array',/error
		return,-1
	endif
	if pcwi_verify_cfg(tcfg,/init,/silent) ne 0 then begin
		pcwi_print_info,ppar,pre,'Target Error - malformed PCWI_CFG struct array',/error
		return,-1
	endif
	;
	...
	for i=0,nmatch-1 do begin
		...
		if nt le 0 then begin
			pcwi_print_info,ppar,pre,'Unrecognized tag',mtags[i],/error
			return,-1
		endif
		tnum[i] = t[0]
	endfor
	...
	for i=0,ncfg-1 do begin
		...
		for j=0,nmatch-1 do begin
			...
			if pcfg[i].(tnum[j]) ne tcfg.(tnum[j]) then begin
				match = (1 eq 0)
			endif
		endfor
		;
		...
		if match then begin
			...
			if keyword_set(imgtype) then begin
				if strmatch(strtrim(pcfg[i].imgtype,2),imgtype) ne 1 then $
					mstat[i] = 0
			endif
			;
			...
			if strlen(tobj) gt 0 then begin
				...
				if strmatch(strtrim(pcfg[i].imgtype,2),'object') eq 1 then begin
					;
					; does the string appear?
					if strpos(pcfg[i].object,tobj) lt 0 then $
						mstat[i] = 0
		...
		;
		; time offset
		tdelt[i] = tcfg.juliandate - pcfg[i].juliandate
		;
		; sky coord offset
		if pcfg[i].ra ge 0. and pcfg[i].dec ge -90. then $
			gcirc,2,tcfg.ra,tcfg.dec,pcfg[i].ra,pcfg[i].dec,dis $
		else	dis = -99.
		cdelt[i] = dis
	endfor
	;
	...
	if count le 0 then begin 
		...
		if not keyword_set(silent) then $
			pcwi_print_info,ppar,pre,'No matches to image,type',tcfg.imgnum,tcfg.imgtype, $
				format='(a,i9,2x,a)',/error
	endif else mcfg = pcfg[mcfgi]
	;
	return,mcfg
end	; PCWI_MATCH_CFG
